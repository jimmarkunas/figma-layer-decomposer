...
def run(manifest_path: Path, schema_path: Path, source_path: Path, mask_path: Path, target_id: str, run_dir: Path, backend=None) -> dict:
    ...
    report = {"mockup_id": manifest["mockup_id"], "target_id": target_id, "pipeline_version": manifest["version"], "source_sha256": sha256(source_path), "output_sha256": sha256(run_dir / "candidate.png"), "canvas": {"width": canvas[0], "height": canvas[1]}, "core_bounds": core.as_dict(), "approved_zone": zone.as_dict(), "changed_pixels_inside_zone": inside_count, "changed_pixels_outside_zone": outside_count, "automated_gate": "PASS" if outside_count == 0 else "FAIL", "human_gate": "PENDING", "notes": []}
